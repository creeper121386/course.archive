...
%token <type_id> ID WRONGID RELOP TYPE    /*指定ID,RELOP 的语义值是type_id，有词法分析得到的标识符字符串*/
...
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE LAST_ARRAY_DIM ARRAY_DIMS ARRAY_DEC ARRAY_ID VAR_DEC ARRAY_ACCESS
...
// for exp2:
program: ExtDefList    {printf("\n\033[1;32m══════════════\nProgram start:\n══════════════\n\033[0m\n");if(SHOW_AST) display($1,0); if (DO_SEMANTIC) semantic_Analysis0($1);}     //显示语法树,语义分析
         ; 
ExtDefList: {$$=NULL;}
...
Arraylist:  LB Exp RB                 {$$=mknode(2,ARRAY_DIMS,yylineno,$2, NULL);}
          | LB Exp RB Arraylist       {$$=mknode(2,ARRAY_DIMS,yylineno,$2,$4);}
        ;
// Arraylist:  LB Exp RB                 {$$=mknode(0,LAST_ARRAY_DIM,yylineno);$$->type_int=$2;}
//           | LB Exp RB Arraylist       {$$=mknode(1,ARRAY_DIMS,yylineno,$4);$$->type_int=$2;}
//         ;
// -------------------------------------------------------------------------------------------
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        ...
        | ID Arraylist ASSIGNOP Exp {$$=mknode(1,ARRAY_DEC,yylineno,$2);strcpy($$->type_id,$1);} 
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}
      ...
      // 数组表达式访问
//       | Exp LB Exp RB    {$$=mknode(2,ARRAY_ACCESS,yylineno,$1,$3);}
